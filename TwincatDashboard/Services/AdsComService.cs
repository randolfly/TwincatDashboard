...
public class AdsComService : IDisposable {
  ...
  /// <summary>
  ///     获取所有可行的Symbols 
  ///     <para>1. 通过SymbolLoader加载所有Symbols</para>
  ///     <para>2. 递归遍历所有Symbols(仅获取MAIN和GVL下的Symbols) </para>
  ///     <para>3. 将Symbol转化为SymbolTree，同时执行重复项剔除 </para>
  ///     <para>4. 将SymbolTree转化为<see cref="List{SymbolInfo}" />，返回</para>
  /// </summary>
  /// <returns>
  ///     可采集数据列表 <see cref="List{SymbolInfo}" />
  /// </returns>
  public List<SymbolInfo> GetAvailableSymbols(List<string> ReadNamespace) {
    ...
    foreach (var symbol in symbols)
      if (ReadNamespace.Contains(symbol.InstanceName, StringComparer.CurrentCultureIgnoreCase))
    ...
    List<SymbolInfo> LoadSymbolTreeBfs(ISymbol root) {
      ...
      while (transverseOrder.Count > 0) {
        ...
        if (symbol.SubSymbols.Count == 0) symbolInfos.Add(new SymbolInfo(symbol));
      }
  ...
  /// <summary>获取连接的Ads server上可用的symbol namespace</summary>
  /// <returns>namespace 列表</returns>
  public List<string> GetAvailableNamespaces() {
    var availableNamespace = new List<string>();
    var settings = new SymbolLoaderSettings(SymbolsLoadMode.VirtualTree,
        ValueAccessMode.SymbolicByHandle);
    var symbolLoader = SymbolLoaderFactory.Create(_adsClient, settings);
    var symbols = symbolLoader.Symbols;

    foreach (var symbol in symbols) availableNamespace.Add(symbol.InstanceName.ToUpper());
  ...
  public async Task<object?> ReadPlcSymbolValueAsync(string symbolPath, Type type) {
    ...
    if (!resultHandle.Succeeded) {
      Log.Error("Failed to create variable handle for {SymbolPath} with error: {ErrorCode}", symbolPath,
          resultHandle.ErrorCode);
      return null;
    }
